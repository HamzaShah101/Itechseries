...
const header = (name, value, override) => options => {
    const headerValue = headers.getHeader(name, options);
...
const standardApi = (name, domainPrefix) => {
    return {
        local: {
...
const validateStatus = options => onResponse(response => response.status >= 200 && response.status < 300 ? response : Promise.reject((0, _response.responseError)(response, `Request for ${options.url} failed with status ${response.status}. ${response.statusText || ''}`)))(options);
...
const enableMigrationCheckBypass = options => query({
