...
import {
    ICON_SIZES
} from './constants/IconSizes';
import themePropType from '../utils/themePropType';
import {
    getIconColor
} from './theme/iconThemeOperators';

const getIconSizeStyles = size => css(["font-size:", ";height:", ";width:", ";"], ICON_SIZES[size] ? `${ICON_SIZES[size]}px` : size, ICON_SIZES[size] ? `${ICON_SIZES[size]}px` : size, ICON_SIZES[size] ? `${ICON_SIZES[size]}px` : size);

const IconWrapper = styled.div.withConfig({
    displayName: "VizExIcon__IconWrapper",
    componentId: "ydgtza-0"
